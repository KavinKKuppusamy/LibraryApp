	...
	if(request.method == "POST"):
		if('checkin' in request.POST):
			...
			for keyword in keywords:
				...
				if(comparision != ""):
					comparision += " AND "
				comparision += "(BkAthr.Isbn LIKE '"+keyword+"' OR lower(BkAthr.Title) LIKE '"+keyword+"' OR lower(BkAthr.authors) LIKE '"+keyword+"' OR BkAthr.Card_id LIKE '"+keyword+"' OR lower(BkAthr.Bname) LIKE '"+keyword+"' OR BkAthr.Ssn LIKE '"+keyword+"')"
			...
			print(books)
			return render(request,'bookcheckin/index.html',{'books':books,'message':message,'get':get})
			...
			cursor.execute(query)
			get=True
			message += "Successfully checked in the book."
			return render(request,'bookcheckin/index.html',{'Books':books,'message':message, 'get':get})
		else:
